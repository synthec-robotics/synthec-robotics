<div align="center">
  <img src="img/Logo-SYNTHEC.png" alt="SYNTHEC Logo" width="200"/>
  <h1>SYNTHEC Robotics</h1>
  <p><strong>Laboratorio de Investigación y Desarrollo | Tecnológico de Monterrey</strong></p>
  
  <a href="https://instagram.com/synthec.robotics">
    <img src="https://img.shields.io/badge/Instagram-%23E4405F.svg?style=for-the-badge&logo=Instagram&logoColor=white" alt="Instagram"/>
  </a>
  <a href="mailto:contacto.synthec.robotics@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/>
  </a>
  <a href="https://www.youtube.com/@synthec.robotics">
    <img src="https://img.shields.io/badge/YouTube-FF0000?style=for-the-badge&logo=youtube&logoColor=white" alt="YouTube"/>
  </a>
</div>

---

### 🔭 Sobre Nosotros

Somos **SYNTHEC**, un equipo de ingeniería multidisciplinario nacido en el **Tec de Monterrey**, Campus Monterrey. Nos dedicamos a explorar las fronteras de la **Industria 5.0**, enfocándonos en la robótica modular, la inteligencia de enjambre y la materia programable.

Nuestra misión es democratizar el acceso a tecnologías de manufactura avanzada, creando sistemas que sean resilientes, reconfigurables y centrados en la colaboración humano-máquina.

---

### 🚀 Proyecto Insignia

<div align="center">
  <a href="LINK_AL_REPOSITORIO_DE_SYMA">
    <img src="img/Logo-SYMA.png" alt="SYMA Banner" width="600"/>
  </a>
  <br>
  <b>SYMA (Sistema Modular Auto-ensamblable)</b>
</div>

> Una plataforma de robótica modular de bajo costo inspirada en la arquitectura SMORES-EP. SYMA es un enjambre de robots capaces de locomoción independiente y auto-reconfiguración mediante acoplamiento magnético activo. 
>
> [**Explorar el Código Fuente y Documentación »**](LINK_AL_REPOSITORIO_DE_SYMA)

---

### 🛠️ Tech Stack & Herramientas

Utilizamos un enfoque integral de hardware y software para dar vida a nuestros robots:

| **Dominio** | **Tecnologías** |
| :--- | :--- |
| **Microcontroladores** | ![ESP32](https://img.shields.io/badge/ESP32-Espressif-red) ![Arduino](https://img.shields.io/badge/-Arduino-00979D?style=flat&logo=Arduino&logoColor=white) |
| **Lenguajes** | ![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=c%2B%2B&logoColor=white) ![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white) |
| **Diseño 3D & CAD** | ![Fusion 360](https://img.shields.io/badge/Fusion%20360-F57F17?style=flat&logo=autodesk&logoColor=white) |
| **Comunicaciones** | ![ESP-NOW](https://img.shields.io/badge/Protocol-ESP--NOW-blue) |
| **Manufactura** | Impresión 3D (PETG/PLA), Diseño de PCB |

---

### 👥 El Equipo

**SYNTHEC** está conformado por estudiantes de ingeniería apasionados por la innovación:

<table>
  <tr>
    <td align="center"><a href="LINK_GITHUB_ANGEL"><b>Angel Salazar</b></a><br>Líder de Proyecto</td>
    <td align="center"><a href="LINK_GITHUB_CARLOS"><b>Carlos García</b></a><br>Sistemas Mecánicos</td>
    <td align="center"><a href="LINK_GITHUB_SUSANA"><b>Susana Fang</b></a><br>Sistemas Electrónicos</td>
    <td align="center"><a href="LINK_GITHUB_EMILIANO"><b>Emiliano Bustamante</b></a><br>Investigación y Documentación</td>
  </tr>
</table>

---

<div align="center">
  <p><i>"Creando el futuro, bloque a bloque."</i></p>
  <p>© 2025 SYNTHEC Robotics. Monterrey, N.L., México.</p>
</div>
